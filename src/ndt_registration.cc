...
int main(int argc, char **argv)
{
    ...
    try {
        TCLAP::CmdLine cmd("NDT registration", ' ', "1.0");
        ...
        TCLAP::ValueArg<double> radius_arg("r", "radius", "The size of the grid", false, 3,
                                           "double", cmd);
    ...
    std::cout << "Size of the grid: " << radius << std::endl;
    ...
    if (ground_truth) {
        double mean_error_before = point_cloud_registration::calculateMSE(source_ground_truth,
                                                                          source_cloud);
        double mean_error_after = point_cloud_registration::calculateMSE(source_ground_truth,
                                                                         aligned_source);
