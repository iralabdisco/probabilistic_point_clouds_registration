...
int main(int argc, char **argv)
{
    ...
    for (double angle = 0.08; angle < 3.14; angle += 0.08) {
        ...
        pcl::transformPointCloud (*source_cloud, *target_cloud, trans);
        registration = std::make_unique<PointCloudRegistration>(target_cloud, source_cloud, params,
                                                                source_cloud);
        registration->align();
        auto estimated_transform = registration->transformation();
        pcl::transformPointCloud (*target_cloud, *aligned_source, estimated_transform);
        double mse_ground_truth = point_cloud_registration::calculateMSE(aligned_source,
                                                                         source_cloud);
        report_file << angle << ", " << mse_ground_truth << std::endl;
