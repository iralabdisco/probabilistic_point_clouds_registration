...
class ProbPointCloudRegistrationIteration {
 public:
  ProbPointCloudRegistrationIteration(
    ...
    for (size_t i = 0; i < data_association.outerSize(); i++) {
      for (Eigen::SparseMatrix<double, Eigen::RowMajor>::InnerIterator it(
        ...
        ErrorTerm *error_term =
            new ErrorTerm(source_cloud[it.row()], target_cloud[it.col()]);
    ...
    weight_updater_callback_.reset(new WeightUpdaterCallback(
        &data_association_, &parameters_, &error_terms_, &weight_updater_,
